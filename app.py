try:
    ...
    app = Flask(__name__)
    app.secret_key = os.environ.get('SECRET_KEY', 'kjsdlsalkdflaflal')
    ...
    def edit_user():
        if request.method == 'POST':
            is_updated, err = handleRegister.updateUserProfile(
    ...
    def public_user_profile(username):
        ...
        if current_user:
            following_list = handleConnection.get_following(
                current_user.get('id'))
            following_id_list = list(
                map(lambda x: x.get('id'), following_list))
